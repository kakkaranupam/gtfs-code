...
SELECT TIME_TO_SEC(TIMEDIFF(TIME(TU.FETCH_TIME), TIME(TU.START_TIME))), TU.* FROM GTFS.TRIP_UPDATES TU 
WHERE TU.STOP_ID = '103905' 
AND TIME_TO_SEC(TIMEDIFF(TIME(TU.FETCH_TIME), TIME(TU.START_TIME))) < 0
...
-- HEATMAP MAYBE?
SELECT S.STOP_ID, COUNT(DISTINCT TRIP_ID), MAX(ARRIVAL_DELAY), MIN(ARRIVAL_DELAY), AVG(ARRIVAL_DELAY), MAX(DEPARTURE_DELAY), MIN(DEPARTURE_DELAY), AVG(DEPARTURE_DELAY)
FROM GTFS.TRIP_UPDATES TU, GTFS.STOPS S
WHERE TU.STOP_ID = S.STOP_ID
AND UPPER(S.STOP_NAME) LIKE '%AMSTERDAM,%'
GROUP BY S.STOP_ID;